...
float3 computeRefraction(float3 V, float ior, thread bool& reflected)
{
    reflected = false;
...
BSDF sampleLambertianBRDF(float3 V, float3 albedo, float2 random, thread float3& L, thread float& pdf)
{
...
BSDF evaluateGGXSmithBRDF(float3 V, float3 L, float3 f0, float roughness, float anisotropy, thread float3& F)
{
...
// NOTE: See 2018, appendix B.
BSDF sampleGGXSmithBRDF(float3 V, float3 f0, float roughness, float anisotropy, float2 random, thread float3& L, thread float& pdf)
{
    // Convert roughness and anisotropy to alpha values.
    float2 alpha = roughnessToAlpha(roughness, anisotropy);
