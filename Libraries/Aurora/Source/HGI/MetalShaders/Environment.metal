...
struct Environment
{
    ...
    Texture2D<float, access::sample> backgroundTexture;
    SamplerState sampler;
    Texture2D<float, access::sample> lightTexture;
#if DIRECTX
...
#if __METAL__
uint3 DispatchRaysIndex() const
{
    return uint3(gRaysIndex, 1);
}
uint3 DispatchRaysDimensions() const
{
    return uint3(gRaysDimensions, 1);
}
#endif // __METAL__
...
float2 computeEnvironmentUV(float4x4 transform, bool useScreen, float3 direction)
{
    ...
    else
    {
        ...
        direction = normalize((transform * float4(direction, 0.0f)).xyz);
...
float3 evaluateEnvironmentTexture(float2 uv, Texture2D<float, access::sample> texture, SamplerState sampler)
{
    // Sample the texture, or interpolate the gradient colors using the Y coordinate.
    return texture.sample(sampler, uv, 0.0f).rgb;
}
...
float3 sampleEnvironment(Environment environment, float2 random, thread float3& L, thread float& pdf)
{
    ...
    if (constants.hasLightTex)
    {
        ...
        uint width = environment.lightTexture.get_width();
        uint height = environment.lightTexture.get_height();
        ...
        uint index       = min(uint(count * random[0]), count - 1);
        AliasEntry entry = gEnvironmentAliasMap[index];
        if (entry.probability < random[1])
        {
            index = entry.alias;
            entry = gEnvironmentAliasMap[index];
        }
        pdf = entry.pdf;
        ...
        L       = normalize((constants.lightTransformInv * float4(L, 0.0f)).xyz);
        float2 uv = computeEnvironmentUV(constants.lightTransform, constants.backgroundUseScreen, L);
        ...
        return environment.lightTexture.sample(gDefaultEnvSampler, uv, 0.0f).rgb;
    }
    else
    {
