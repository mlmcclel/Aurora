...
{
    return mx_ggx_directional_albedo_curve_fit(NdotV, roughness, float3(F0), float3(F90)).x;
}
...
{
    return mx_ggx_energy_compensation(NdotV, roughness, float3(Fss)).x;
}
...
void mx_artistic_to_complex_ior(
                                float3 reflectivity, float3 edge_color, thread float3& ior, thread float3& extinction)
{
...
static constexpr constant float u_sheenAlbedo[SHEEN_ALBEDO_TABLE_SIZE * SHEEN_ALBEDO_TABLE_SIZE] = { 1.6177,
...
{
    float rotationRadians = amount * M_PI / 180.0;
    float4x4 m            = mx_rotationMatrix(axis, rotationRadians);
    return (m * float4(vec, 1.0)).xyz;
}
                                 ...
                                 Material material, ShadingData shading, float3 Vw, float3 Lw)
{
    ...
    float coat_clamped                    = clamp(material.coat, 0.0f, 1.0f);
...
float3 calculateHemisphereLightDirection(
                                         float3 tangent, float3 bitangent, float3 normal, float3 V, float2 random, thread float& pdf)
{
...
float3 calculateGGXLightDirectionAndPDF(float3 tangent, float3 bitangent, float3 normal, float3 V,
                                        float2 alpha, float2 random, thread float3& Lw, thread float& pdf)
{
...
BSDF_AND_COSINE sampleMaterialAllLobes(Material material, ShadingData shading, float3 Vw,
                                       float2 random, thread float3& Lw, thread float& pdf, thread int& lobeID)
{
    lobeID = 1;
...
BSDF_AND_COSINE evaluateMaterialAndPDF(Material material, ShadingData shading,
                                       float3 Vw, float3 Lw, float2 random, thread float& pdf, thread int& lobeID)
{
